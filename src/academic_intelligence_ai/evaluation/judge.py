...
- "boolean": ocena 3 ako je da/ne odgovor tačan — detalji nisu obavezni za pun bod
- "factual": strogo proveravaj tačnost konkretnih podataka (emailovi, telefoni, imena)
- "procedural": ocena 3 ako su navedeni ispravni koraci, čak i bez svih detalja
...
class Judge:
    ...
    def _parse_result(self, result: dict, key: str) -> tuple[int, str, int]:
        ...
        try:
            ...
            logger.warning("Failed to parse judge response for %s: %s | raw: %s", key, e, result.get("answer", ""))
